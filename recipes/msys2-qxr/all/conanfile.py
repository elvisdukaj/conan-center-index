...
class lock:
    def __init__(self):
        self.handle = ctypes.windll.kernel32.CreateMutexA(None, 0, "Global\\ConanMSYS2-QXR".encode())
        if not self.handle:
...
class MSYS2QxrConan(ConanFile):
    ...
    default_options = {
        ...
        "packages": "base-devel,binutils,gcc,python-pip,ruby",
    ...
    def _install_jinja2(self):
        self.run(f'bash -l -c "python -m pip install jinja2"')

    def _prepare_python_env(self):
        self._install_jinja2()

    def _get_asciidoc_gems_path(self) -> PurePosixPath:
        stdout_stream = io.StringIO()
        self.run('bash -l -c "echo $(gem environment user_gemhome)"', stdout=stdout_stream)
        gems_path = stdout_stream.getvalue().strip()
        self.output.info(f"gems path: {gems_path}")
        return PurePosixPath(gems_path)

    def _create_sym_link_asciidoc(self, gems_path: PurePosixPath) -> None:
        asciidoctor_name = "asciidoctor"
        asciidoctor_src = PurePosixPath(gems_path, "bin", asciidoctor_name)
        asciidoctor_dst = PurePosixPath("/", "usr", "bin", asciidoctor_name)
        self.run(f'bash -l -c "ln -s {asciidoctor_src} {asciidoctor_dst}"')

    def _install_asciidoctor_gem(self) -> None:
        self.run(f'bash -l -c "gem install asciidoctor"')

    def _prepare_asciidoctor_env(self):
        self._install_asciidoctor_gem()
        gems_path = self._get_asciidoc_gems_path()
        self._create_sym_link_asciidoc(gems_path)

    def _prepare_qxr_environment(self):
        with chdir(self, os.path.join(self._msys_dir, "usr", "bin")):
            self._prepare_python_env()
            self._prepare_asciidoctor_env()
